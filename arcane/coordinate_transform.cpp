...
private:
    ...
    unsigned int wgtochina_lb(
            int wg_flag, unsigned int wg_lng, unsigned int wg_lat, 
...
const double _LL2RAD_ = 0.0174532925194; const long double _OFFSET_X_ = 0.0065;
...
unsigned int GcjEncryptor::wgtochina_lb(
        int wg_flag, unsigned int wg_lng, unsigned int wg_lat, 
        int wg_heit, int wg_week, unsigned int wg_time,
        unsigned int* china_lng, unsigned int* china_lat) {
    double x_add;
...
int gcj_decrypt(dpoint_t oldxy, dpoint_t* newxy) {
    ...
    double xmidnew = 0.0, ymidnew = 0.0;
    double gpsx, gpsy;
    int res = -1;
...
int coordtrans(
    ...
    if (strncmp(from, to, std::min(from_len, to_len) - 2) != 0) {
